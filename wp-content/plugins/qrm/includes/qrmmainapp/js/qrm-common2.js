...
function setMatrix(tolString, maxImpact, maxProb, val, svgDivID, treated, clkCallBack) {
    ...
    var svg = d3.select(svgDivID).append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");
    var heatMap = svg.selectAll(".hour").data(data).enter().append("g").attr("class", "tolCell").attr("id", function (d) {
        return d.impact + "-" + d.prob;
    });
    heatMap.append("rect").attr("x", function (d) {
        return (d.impact - 1) * gridSizeX;
    }).attr("y", function (d) {
        return (maxProb - d.prob) * gridSizeY;
    }).attr("rx", 2).attr("ry", 2).attr("class", function (d) {
    ...
    }).attr("width", gridSizeX).attr("height", gridSizeY).on("click", function (d) {
        clkCallBack(d.impact, d.prob, d.treated, d.tol);
    });
    heatMap.append("text").attr("x", function (d) {
        return (d.impact - 1) * gridSizeX + gridSizeX / 2;
    }).attr("y", function (d) {
        return (maxProb - d.prob) * gridSizeY + gridSizeY / 2 + 5;
    }).attr("class", "tolText").attr('pointer-events', 'none').style("text-anchor", "middle").on("click", function (d) {
        clkCallBack(d.impact, d.prob, d.treated, d.tol);
    }).text(function (d) {
...
function setConfigMatrix(tolString, maxImpact, maxProb, svgDivID, matrixChangeCB) {
    ...
    heatMap.append("rect").attr("x", function (d) {
        ...
        return (maxProb - d.prob) * gridSizeY;
    }).attr("rx", 2).attr("ry", 2).attr("qrmID", function (d) {
    ...
    svg.append("text").attr("text-anchor", "middle").style("font-size", "12px").style("font-weight", "normal").attr("transform", "translate(" + [8, gridSizeY * (maxProb - 1) + 12] + ")").text("P1");
    svg.append("text").attr("text-anchor", "middle").style("font-size", "12px").style("font-weight", "normal").attr("transform", "translate(" + [8, gridSizeY * (maxProb - 2) + 12] + ")").text("P2");
    if (maxProb > 2)
        svg.append("text").attr("text-anchor", "middle").style("font-size", "12px").style("font-weight", "normal").attr("transform", "translate(" + [8, gridSizeY * (maxProb - 3) + 12] + ")").text("P3");
    if (maxProb > 3)
        svg.append("text").attr("text-anchor", "middle").style("font-size", "12px").style("font-weight", "normal").attr("transform", "translate(" + [8, gridSizeY * (maxProb - 4) + 12] + ")").text("P4");
    if (maxProb > 4)
        svg.append("text").attr("text-anchor", "middle").style("font-size", "12px").style("font-weight", "normal").attr("transform", "translate(" + [8, gridSizeY * (maxProb - 5) + 12] + ")").text("P5");
    if (maxProb > 5)
        svg.append("text").attr("text-anchor", "middle").style("font-size", "12px").style("font-weight", "normal").attr("transform", "translate(" + [8, gridSizeY * (maxProb - 6) + 12] + ")").text("P6");
    if (maxProb > 6)
        svg.append("text").attr("text-anchor", "middle").style("font-size", "12px").style("font-weight", "normal").attr("transform", "translate(" + [8, gridSizeY * (maxProb - 7) + 12] + ")").text("P7");
    if (maxProb > 7)
        svg.append("text").attr("text-anchor", "middle").style("font-size", "12px").style("font-weight", "normal").attr("transform", "translate(" + [8, gridSizeY * (maxProb - 8) + 12] + ")").text("P8");
}
function hasClassSVG(tmp, oldClass, newClass) {
    //Templated the jQuery hasClass function to work for SVG
    var rclass = /[\t\r\n\f]/g;
    var className = " " + oldClass + " ", i = 0, l = tmp.length;
    for (; i < l; i++) {
...
function setRiskEditorMatrix(risk, matrixConfig, matrixDIVID, matrixDisplayConfig, dragStartCallback, dragCallback, dragEndCallback) {
    ...
    }).attr("width", gridSizeX).attr("height", gridSizeY);
    ...
    var untreatedRisk = svg.selectAll().data([risk]).enter().append("g").style("cursor", "move").attr("transform", "translate(" + [risk.x, risk.y] + ")").attr("treated", false).attr("class", "riskEditorRiskUntreated").call(drag);
    ...
    untreatedRisk.append("text").attr({
        'text-anchor': 'middle',
        y: 4
    }).attr("class", "untreated").style("font-size", "8px").text(function (d) {
        return d.riskProjectCode;
    });
    var treatedRisk = svg.selectAll().data([risk]).enter().append("g").style("cursor", "move").attr("transform", "translate(" + [risk.x1, risk.y1] + ")").attr("treated", true).attr("class", "riskEditorRiskTreated").call(drag);
    ...
    treatedRisk.append("text").attr({
        'text-anchor': 'middle',
        y: 4
    }).attr("class", "treated").style("font-size", "8px").text(function (d) {
        return d.riskProjectCode;
    });
    svg.append("text").attr("text-anchor", "middle").style("font-size", "14px").style("font-weight", "normal").attr("transform", "translate(" + [-10, height / 2] + ") rotate(-90)").text("Probability");
    svg.append("text").attr("text-anchor", "middle").style("font-size", "14px").style("font-weight", "normal").attr("transform", "translate(" + [width / 2, height + 20] + ")").text("Impact");
}
...
d3.gantt = function (calController) {
    ...
    function gantt(tasks, svgdiv, w, h) {
        ...
        initAxis();
        var svg = d3.select(svgdiv).append("svg").attr("class", "chart").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr("class", "gantt-chart").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).attr("transform", "translate(" + margin.left + ", " + margin.top + ")");
        ...
        svg.append("g").attr("class", "x axis").attr("transform", "translate(0, " + (height - margin.top - margin.bottom) + ")").transition().call(xAxis).selectAll("text").style("text-anchor", "end").style("fill", "rgb(103,106,108)").attr("dx", "-.8em").attr("dy", ".15em").attr("transform", function (d) {
            return "rotate(-45)";
        });
        svg.append("text").attr("text-anchor", "middle").style("font-size", "20px").style("font-weight", "normal").style("fill", "rgb(103,106,108)").attr("transform", "translate(" + [width / 2, 0] + ")").text(calController.project.title);
        svg.append("g").attr("class", "y axis").transition().call(yAxis).selectAll("text").style("fill", "rgb(103,106,108)");
        return gantt;
    }
    gantt.redraw = function (tasks) {
        ...
        rect.enter().insert("rect", ":first-child").attr("rx", 5).attr("ry", 5).attr("class", "bar").transition().attr("y", 0).attr("transform", rectTransform).attr("height", function (d) {
            return y.rangeBand();
        }).attr("width", function (d) {
            return (x(d.endDate) - x(d.startDate));
        });
...
function SorterLayout(rankCtl, $scope) {
    ...
    var layout = this;
    this.drag = d3.behavior.drag().on("dragstart", function (d) {
        var e = d3.event.sourceEvent;
        if (e.ctrlKey)
            return;
        var x = d.tx + 5;
        var y = d.ty + 5;
        d3.select("#rankGroupHolder").append("use").attr("xlink:href", "#ghostDef").attr("tx", x).attr("ty", y).attr("totTy", d.totTy).attr("transform", "translate(" + x + ", " + y + ")").attr("id", "sorterGhost");
        d3.select("#rankGroupHolder").append("use").attr("xlink:href", "#insertionMarkerDef").attr("ty", y).attr("tx", x).attr("transform", "translate(" + x + ", " + y + ")").attr("id", "insertionMarker").style("display", "none");
        this.parentNode.appendChild(this);
    }).on("drag", function () {
        ...
        }).attr("totTy", totTy).attr("ty", y).attr("tx", x).style("display", "inline");
    }).on("dragend", function (d) {
    ...
    this.layoutTable = function () {
        ...
        this.topSVG = d3.select(this.svgDiv).append("svg").attr("width", this.width).attr("height", this.height);
        this.createDefinitions();
        this.topSVG.append("text").attr("text-anchor", "middle").style("font-size", "20px").style("fill", "rgb(103,106,108)").style("font-weight", "normal").attr("transform", "translate(" + [this.width / 2, 20] + ")").text(rankCtl.project.title);
        this.svg = this.topSVG.append("g").attr("id", "rankGroupHolder").attr("h", this.height).attr("transform", "matrix(" + this.transMatrix.join(' ') + ")");
        var me = this;
        var rowNum = 0;
        var colNum = 0;
        this.items.forEach(function (item) {
        ...
        var risk = nodes.enter().append("g").attr("class", "risk").attr("id", function (d) {
            return d.riskCode;
        }).attr("tx", function (d) {
            return d.tx;
        ...
        risk.append("rect").attr("width", this.itemWidth - 10).attr("height", this.itemHeight - 5).attr("fill", 'lightgray').style("stroke", 'gray').attr("class", 'tolText').on("mouseover", function (d) {
            rankCtl.showDesc = true;
            rankCtl.showInstructions = false;
            rankCtl.displayRisk = d;
            $scope.$apply();
        ...
        risk.append("rect").attr("width", "70").attr("height", this.itemHeight - 8).attr("class", function (d) {
            var tol = null;
            if (d.treated) {
        ...
        risk.append("text").style("pointer-events", "none").attr("transform", "translate(10,25)").text(function (d) {
            return d.riskProjectCode;
        });
        var textGroup = risk.append("g").attr("transform", "translate(80,25)");
        textGroup.append("clipPath").attr("id", function (d) {
            return "clipPath" + d.riskProjectCode;
        }).append("rect").attr("transform", "translate(0,-20)").attr("width", this.itemWidth - 90).attr("height", this.itemHeight);
        textGroup.append("text").style("pointer-events", "none").attr("clip-path", function (d) {
...
function initData(QRMDataService, remoteService, $q) {
    var intro = {};
    a = remoteService.getSiteUsers().then(function (response) {
    ...
    b = remoteService.getCurrentUser().then(function (response) {
        if (response.data == "0" || response.data == "-1") {
            intro.sessionOK = false;
        }
        else {
    ...
    c = remoteService.getAllRisks().then(function (response) {
        if (response.data == "0" || response.data == "-1") {
            intro.sessionOK = false;
        }
        else {
    ...
    d = remoteService.getProjects().then(function (response) {
        if (response.data == "0" || response.data == "-1") {
            intro.sessionOK = false;
        }
        else {
...
//# sourceMappingURL=qrm-common2.js.map